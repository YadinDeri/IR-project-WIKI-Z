...
class MultiFileReader:
    ...
    def __init__(self):
        self._open_files = {}
        self.client=storage.Client()
        self.bucket = self.client.bucket('ir-project-z')

    def read(self, locs, n_bytes,kind):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(Path(f'./postings_gcp_{kind}') / f'{f_name}', 'rb')
                # ---- with our bucket ----
                #f_name = my_bucket.get_blob(f_name)
                # blob = self.bucket.get_blob(f'postings_gcp_{kind}/{f_name}')
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        self.dominator_mapping = defaultdict(list)
